const vm = __addon.tab.traps.vm; // Must be in the Editor! Otherwise, Error!
 /**
...
},{"./plurals":3,"format-message-formats":1,"lookup-closest-locale":6}],3:[function(require,module,exports){
// @flow
'use strict'
...
},{"format-message-formats":1,"format-message-interpret":2,"format-message-interpret/plurals":3,"format-message-parse":4,"lookup-closest-locale":6}],6:[function(require,module,exports){
// @flow
...
window.createTranslate = createTranslate;
...
/**
 * Sets up the window.Scratch API for an unsandboxed extension.
...
const setupUnsandboxedExtensionAPI = vm => new Promise(resolve => {
    ...
    // Create a new copy of window.Scratch for each extension
    const Scratch = Object.assign({}, window.Scratch || {}, ScratchCommon);
    Scratch.extensions = {
        unsandboxed: true,
        register
    };
    ...
    window.ScratchExtensions = createScratchX(Scratch);

    vm.emit('CREATE_UNSANDBOXED_EXTENSION_API', Scratch);
    console.log("Scratch Unsandboxed API Ready!");
});

/**
 * Disable the existing window.Scratch unsandboxed extension APIs.
 * This helps debug poorly designed extensions.
 */
const teardownUnsandboxedExtensionAPI = () => {
    // We can assume window.Scratch already exists.
    window.Scratch.extensions.register = () => {
...
const loadUnsandboxedExtension = (extensionURL, vm) => new Promise((resolve, reject) => {
    // You must wait for the setup API before creating the script.
    setupUnsandboxedExtensionAPI(vm).then(() => {
        const script = document.createElement('script');
        script.onload = () => {
            // This is the correct place to resolve the promise.
            // It will only happen after the script has loaded.
...
// Because loading unsandboxed extensions requires messing with global state (window.Scratch),
// only let one extension load at a time.
const limiter = new AsyncLimiter(loadUnsandboxedExtension, 1);
const load = (extensionURL) => limiter.do(extensionURL, Scratch.vm);
setupUnsandboxedExtensionAPI(vm); // Make it accessible
print("Waiting for Scratch Object...");
function addCustomLoaderMessage() {
  const container = document.querySelector(
    '#app .gui .loader_background_KvT9o .loader_message-container-inner_pQs1M'
  );
  if (!container) {
  ...
  newDiv.className = 'loader_message_4Xz7-';

  const span = document.createElement('span');
  span.textContent = 'Creating the Scratch...';

  newDiv.appendChild(span);
  container.appendChild(newDiv);

  console.log('[UI] Custom loader message added');
}

let createAPIReady = false;
let projectLoadedArgs = null;

// Listen for CREATE_UNSANDBOXED_EXTENSION_API
vm.once('CREATE_UNSANDBOXED_EXTENSION_API', () => {
  createAPIReady = true;
  console.log('[Hook] CREATE_UNSANDBOXED_EXTENSION_API fired');

  // If PROJECT_LOADED already happened, emit it now
  if (projectLoadedArgs) {
    vm.emit('PROJECT_LOADED', ...projectLoadedArgs);
    projectLoadedArgs = null;
  }
});

// Intercept PROJECT_LOADED
vm.on('PROJECT_LOADED', (...args) => {
  if (!createAPIReady) {
    console.log('[Hook] PROJECT_LOADED intercepted, waiting for CREATE_UNSANDBOXED_EXTENSION_API');
    addCustomLoaderMessage();
    projectLoadedArgs = args;
    return; // stop here until ready
  }
});
window.loadUnsandboxed = load;
const loadSandboxed = (ExtensionURL) => {
	Scratch.vm.extensionManager.loadExtensionURL(ExtensionURL);
	console.log("Loaded extension! Note: If it doesnt load, Please report a bug at our github page. This project is still in ALPHA.")
}
window.loadSandboxed = loadSandboxed;
// Script copied from turbowarp gui scratch-vm/tw-unsandboxed-extension-runner.js with major edits and browserify for  format-message
